...
for ( var i = 0; i < allStorage().length - 1; i++ ){
	if ( !isEmpty(allStorage()[i]) ){
		savedData.innerHTML += 
					  `<li class="well well-lg top-right-parent">
					  	  <a target="_blank" class="top-right btn" href="https://duckduckgo.com/?q=!ducky+${allStorage()[i]}">VISIT SITE <span class="glyphicon glyphicon-new-window"></span></a>
                          <h2 class="name">${allStorage()[i]}</h2> <br>
                          <div class="btn-group">
                            <a href="https://www.google.ie/search?q=${allStorage()[i]}+logo&source=lnms&tbm=isch" target="_blank" class="btn btn-primary btn-block">Check on Google</a>
